...
export default function Slider() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      const elements = gsap.utils.toArray(".avis");
  ...
  return (
    <section
      ref={scroller}
      className="scroller py-4 w-full overflow-hidden flex items-center text-50"
    >
      <div ref={wrapper} className="flex">
        {doubledAvis.map((item, index) => (
          <article
            key={index}
            className="avis font-satoshi tracking-satoshi flex gap-4 mx-8 rounded h-fit "
